...
const { insertToken } = require('../module/redis');
...
router.post('/', async(req,res)=>{
    ...
    const queryString = `SELECT user_code,user_id FROM users WHERE user_id = ? AND user_password = ?`;
    ...
    try{
        ...
        if(userdata.length > 0){
            ...
            // 지금은 그대로 넘김.
            const { user_code, user_id } = userdata[0];
            const accessToken = generateAccessToken(user_id);
            const { refreshToken, jti } = generateRefreshToken(user_code, user_id);

            let tokenKey = `refreshtoken:${user_id}:${jti}`
            ...
            await maria.execute(updateJtiQuery, [jti, user_id]);


            // access token은 클라이언트 로컬 저장소에 저장합니다..
            // access token은 쿠키에 저장하는걸로 변경
